...
class ScriptSpec extends Specification {
  ...
  "Script" should {
    ...
    "extract the pubkey from scripSig" in {
    ...
    "extract the pubkey from scriptPubKey" in {
      ...
      val scriptPubKeyBytes = "76a91433e81a941e64cda12c6a299ed322ddbdd03f8d0e88ac".hex2bytes
      val scriptPubKey = Script(scriptPubKeyBytes)

      scriptPubKey.toString === "OP_DUP OP_HASH160 33e81a941e64cda12c6a299ed322ddbdd03f8d0e OP_EQUALVERIFY OP_CHECKSIG "

      val pubKeyHash = new {} with VersionedChecksummed {
        val bytes = scriptPubKey.getPubKeyHash
        val version = 111
      }

      pubKeyHash.toBase58 === "mkFQohBpy2HDXrCwyMrYL5RtfrmeiuuPY2"
