...
class EncodingSpec extends Specification {
  ...
  "Domain objects" should {
    ...
    "encode a tx in bytes" in {
      ...
      val parsed = parse[Transaction](rawTx.hex2bytes,0)
      val parsedHex = bytes2hex(parsed.get._1.byteFormat.toArray)

      val sigScript:Array[Byte] = parsed.get._1.txIn.head.previousOutput.hash
